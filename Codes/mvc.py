# -*- coding: utf-8 -*-
"""
Created on Mon Jun 27 16:33:05 2022
...
def mvc(x,d,sigma,w0):
    ...
    while(n<N):
        ...
        e_c = np.conjugate(e)
        tau = 2#1/tau
        a= 0.5*np.power(tau,2)
        mu=0.01
        BB = 2*tau/(1+tau*np.power((e*e_c),1))
        BB=np.power(BB,2)
        CC= np.absolute(e)*BB*np.sign(e)
        w_t=w_t+1*mu*CC*((xn.T))  #np.conjugate    
        # exponential part
        #a=1
        #tau=np.power(2*a,2)
        
        B = 2*a/(1+tau*np.power((e*e_c),1)) #-0.5*(e*e_c)/(math.pow(sigma,2))
