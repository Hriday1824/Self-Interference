...
# 	plt.title('Self interference Cancellation - LMS')
...
# 	plt.show()  ; LMSy=np.save('LMSy.npy', toPlotyTestLinCancFFT ) ;LMSyn=np.save('LMSyn.npy', toPlotyTestNonLinCancFFT);  RS=np.save('RS.npy', toPlotyTestFFT);  NP=np.save('NP.npy', toPlotnoisefft) 
...
# 	A = np.transpose(np.reshape([np.flip(x[i+1:i+chanLen+1],axis=0) for i in range(x.size-chanLen)], (x.size-chanLen, chanLen)));    y = y[np.newaxis, :]; y=np.matrix(y); y=y[0,chanLen:]
# 	# Solve LS problem

# 	h = LMS(A, y); h=h[:,18411];     h=np.array(h); h=np.squeeze(h,1); h=np.conjugate(h); 
...
# 	sys.stdout.write("\r1. Constructing basis functions... done!         \n");    y = y[np.newaxis, :]; y=np.matrix(y); y=y[0,chanLen:]

# 	# Solve LS problem
# 	sys.stdout.write("2. Doing channel estimation... ");  A=np.transpose(A)
# 	sys.stdout.flush()
# 	h = LMS(A, y,0.01,np.zeros([260,1])); h=h[:,18411];     h=np.array(h); h=np.squeeze(h,1)#np.linalg.lstsq(A, y[chanLen:])[0]
...
def plotPSD(y_test, yCanc, yCancNonLin, noise, params, type, yVar=1):
    ...
    plt.legend(['Received SI Signal ({:.1f} dBm)'.format(yTestPower), 'After Linear Digital Cancellation ({:.1f} dBm)'.format(yTestLinCancPower), 'After Non-Linear Digital Cancellation ({:.1f} dBm)'.format(yTestNonLinCancPower), 'Measured Noise Floor ({:.1f} dBm)'.format(noisePower)], loc='upper center')
    plt.show()
    LMSy = np.save('LMSy.npy', toPlotyTestLinCancFFT)
    LMSyn = np.save('LMSyn.npy', toPlotyTestNonLinCancFFT)
    RS = np.save('RS.npy', toPlotyTestFFT)
    NP = np.save('NP.npy', toPlotnoisefft)

    return noisePower, yTestPower, yTestLinCancPower, yTestNonLinCancPower

def SIestimationLinear(x, y, params):
    chanLen = params['hSILen']
    A = np.transpose(np.reshape([np.flip(x[i + 1:i + chanLen + 1], axis=0) for i in range(x.size - chanLen)], (x.size - chanLen, chanLen)))
    y = y[np.newaxis, :]
    y = np.matrix(y)
    y = y[0, chanLen:]
    h = LMS(A, y)
