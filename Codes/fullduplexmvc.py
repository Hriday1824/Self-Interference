...
def plotPSD(y_test, yCanc, yCancNonLin, noise, params, type, yVar=1):
	...
	plt.title('Self interference Cancellation - MVC')
	...
	plt.show(); MVCy=np.save('MVCy.npy', toPlotyTestLinCancFFT) ;MVCyn=np.save('MVCyn.npy', toPlotyTestNonLinCancFFT); 
...
def SIestimationLinear(x, y, params):
	...
	h = mvc(A, y,0.5,np.zeros([13,1])); h=h[:,18411];     h=np.array(h); h=np.squeeze(h,1); h=np.conjugate(h); 
...
def SIestimationNonLinear(x, y, params):
	...
	sys.stdout.flush()
	h = mvc(A, y,0.01,np.zeros([260,1])); h=h[:,18411];     h=np.array(h); h=np.squeeze(h,1)#np.linalg.lstsq(A, y[chanLen:])[0]
